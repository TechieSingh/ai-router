...
The launcher runs hidden with a 40,960-token context by default — this model's native training ceiling — one generation slot, full GPU offloading, an 8192/4096 batch/ubatch size, Flash Attention, and Q8 KV cache. It binds only to `127.0.0.1:8080`, with model alias `local-coder`. Wait for loading to finish, then click **Connections**. Logs are `.runtime/local.stdout.log` and `.runtime/local.stderr.log`. Active local requests disable thinking through llama.cpp's chat-template option.

Measured on a 12 GB RTX 3060 under an actual sustained generation load (not just at idle, which reads lower): 8K context uses about 7.4 GB VRAM, 28K about 8.4 GB, and the native 40,960 ceiling about 9.35 GB (76%). The batch/ubatch size is raised from llama.cpp's defaults to 8192/4096 to push that to about 10.3 GB (84%), which is a prompt-throughput improvement with no quality tradeoff, unlike context size — 40,960 is the largest context this model can use without RoPE/YaRN scaling (which trades output quality for context beyond the model's training window and is not configured here; this llama.cpp build hard-caps the server context at the model's native size even if YaRN is requested). About 2 GB stays free for Windows and the display at these settings. Full-precision (`f16`) KV cache was also tested and reaches 97% VRAM with llama.cpp's own loader warning it may not fit — too close to the edge to run reliably, so it is not used. To use a smaller context, start with `-Context 8192` or `-Context 16384` and change **Jev Router: Local Context** to match. System RAM is not interchangeable with VRAM.
