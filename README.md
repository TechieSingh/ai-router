...
The launcher runs hidden with a 40,960-token context by default — this model's native training ceiling — one generation slot, full GPU offloading, Flash Attention, and Q8 KV cache. It binds only to `127.0.0.1:8080`, with model alias `local-coder`. Wait for loading to finish, then click **Connections**. Logs are `.runtime/local.stdout.log` and `.runtime/local.stderr.log`. Active local requests disable thinking through llama.cpp's chat-template option.

Measured on a 12 GB RTX 3060 under an actual sustained generation load (not just at idle, which reads lower): 8K context uses about 7.4 GB VRAM, 28K about 8.4 GB, and the native 40,960 ceiling about 9.35 GB (76%) — consistently, with roughly 2.9 GB free for Windows and the display. That is why 40,960 is the shipped default: it is the largest context this model can use without RoPE/YaRN scaling, which trades output quality for context beyond what the model was trained on and is not configured here. To use a smaller context, start with `-Context 8192` or `-Context 16384` and change **Jev Router: Local Context** to match. System RAM is not interchangeable with VRAM.
